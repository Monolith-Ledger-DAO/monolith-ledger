...
// Скопированный ABI из MonolithGovernanceToken.json
const lithAbi = [
...
export function useLITHInfo() {
  const contractConfig = {
    address: contracts.governanceToken.address as Address,
    abi: lithAbi,
  };
  ...
  const formattedTotalSupply = typeof totalSupply === "bigint"
...
export function useUserLITHAccount() {
    ...
    // Читаем баланс LITH пользователя
    const { data: balance, refetch: refetchBalance } = useReadContract({
        ...
        args: [address],
        query: {
            enabled: !!address, // Запрос выполняется только если адрес подключен
        },
    });

    // Читаем, кому пользователь делегировал голоса
    ...
    const { writeContractAsync: delegateVotes, isPending } = useWriteContract();

    const formattedBalance = typeof balance === "bigint"
      ? Number(formatUnits(balance, 18)).toFixed(4)
      : "0.0000";
